...
function getMaxUserId(){
  ...
  function upload_Pdf($max_size) {
          ...
          //-- set upload path
          $config = array(
            'upload_path'  => UPLOAD_FILE . '/documents',
            'upload_url'   => base_url() . UPLOAD_FILE . '/documents',
            'allowed_types'=> '*',
            'max_size'     => $max_size,
          );
          ...
          if ($this->upload->do_upload("file")) {
              ...
              //-- set upload path
              $source = UPLOAD_FILE . "/documents/" . $data['file_name'] ;
              return array(
                  'status' => 1,
                  'path' => $source,
              );
  ...
  public function getIndexTags($root){
    ...
    public function kycDocument($id){
      return $this->db->select('type, details, created')->get_where('documents', ['root' => $id, 'deleted' => 0])->result_array();
    }
    public function get_bank_list($id){
      return $this->db->get_where('banks', ['root' => $id, 'deleted' => 0])->result_array();
    }
}
