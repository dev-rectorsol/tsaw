...
class User extends CI_Model
{
    ...
    {
        return $this->db->get_where("logme", array("logid" => $id, "deleted" => 0))->row(0);
    }
    ...
    public function all()
    {
        return $this->db->get_where("logme", array("deleted" => 0))->result();
    }
    ...
    {
        return $this->db->where($field,$id)->update('logme', $action);
    }
    ...
    public function userKycstatus($id){
      return $this->db->select('kyc')->get_where("logme", array("logid" => $id))->row(0);
    }
    public function userKycUpdate($id, $action){
      return $this->db->where("logid", $id)->update('logme', $action);
    }
    public function executorCount($parent){
      return $this->db->get_where("logme", ['parent' => $parent, 'role_id' => 3])->num_rows();
    }
    public function pendingExecutorCount(){
      return $this->db->get_where("logme", ['role_id' => 3])->num_rows();
    }
}
